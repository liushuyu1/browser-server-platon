...
public class WebAccessFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain chain) throws IOException, ServletException {
        ...
        ((HttpServletResponse) servletResponse).setHeader(CommonConstant.TRACE_ID, traceId);
        log.info("[请求接口结束] http状态:{}, 耗时:{}ms", CommonUtil.ofNullable(() -> ((HttpServletResponse) servletResponse).getStatus()).orElse(-1), System.currentTimeMillis() - start);
    }
     ...
     * @date 2021/4/23
     */
    private String getReqTraceId(RequestBodyWrapper requestWrapper) {
        String traceId = "";
        try {
     ...
     * @date 2021/4/23
     */
    private void requestParamLog(RequestBodyWrapper requestWrapper) {
